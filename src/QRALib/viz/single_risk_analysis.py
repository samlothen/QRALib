...
def plot_single_risk(
    ...
    # Exceedance curve
    fig.add_trace(
        go.Scatter(x=bins, y=exc, mode="lines", name="Exceedance"),
        row=1, col=1
    )
    # Frequency boxplot
    fig.add_trace(
        go.Box(x=freq, name="Frequency", boxmean="sd", showlegend=False),
        row=1, col=2
    )
    # Impact boxplot
    fig.add_trace(
        go.Box(x=imp_ppf, name="Impact", boxmean="sd", showlegend=False),
        row=2, col=2
    )
    # Statistics table
    ...
    values  = [
        ["Frequency","Impact"],
        tbl["min"], tbl["p5"], tbl["mean"], tbl["p95"], tbl["max"]
    ]
    fig.add_trace(
        go.Table(header=dict(values=headers, align="left"),
                 cells=dict(values=values, align="left")),
        row=3, col=1
    )

    fig.update_layout(
        title_text=f"Single Risk Analysis — {risk_id}",
        height=750,
        width=1000
    )
    return fig
